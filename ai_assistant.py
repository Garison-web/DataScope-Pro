...
def ask_with_history(question: str, df: pd.DataFrame, history: list, use_cache: bool = False) -> str:
    """Ask with conversation history for follow-up awareness."""
    client = get_client()
    ...
    system_prompt = (
        ...
        "You have access to the recent conversation history — use it for follow-up questions. "
    ...
    # Build history string (last 6 exchanges max)
    history_str = ""
    if history:
    ...
    for model in MODELS:
        for attempt in range(3):
            try:
                ...
                break
    raise RuntimeError(
        f"Gemini unavailable. (Last error: {last_err})"
    )
